...
const recipe_3 = __importDefault(require("../../anomalydetection/recipe"));
function getAPIImplementation() {
    return {
        ...
        signInPOST: async function ({ formFields, tenantId, session, options, userContext }) {
            ...
            if (utils_2.isFakeEmail(email) && session === undefined) {
            ...
            const anomalyDetection = recipe_3.default.getInstance();
            ...
            if (utils_2.isFakeEmail(email) && preAuthChecks.isFirstFactor) {
                // we will intercept this request in favor to detect brute force attacks
                if (anomalyDetection !== undefined) {
                    await anomalyDetection.recipeInterfaceImpl.checkAnomalyWithRequest({
                        request: options.req,
        ...
        signUpPOST: async function ({ formFields, tenantId, session, options, userContext }) {
            ...
            const preAuthCheckRes = await authUtils_1.AuthUtils.preAuthChecks({
                ...
                factorIds: ["emailpassword"],
                isSignUp: true,
                isVerified: utils_2.isFakeEmail(email),
                signInVerifiesLoginMethod: false,
                skipSessionUserUpdateInCore: false,
            ...
            if (utils_2.isFakeEmail(email) && preAuthCheckRes.isFirstFactor) {
                // Fake emails cannot be used as a first factor
                return {
                    status: "EMAIL_ALREADY_EXISTS_ERROR",
                };
